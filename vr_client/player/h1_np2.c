...
int BUFFER_LIMIT = 2; // initial buffer limit
...
double z1_bit[100], z2_bit[100], z3_bit[100];
char *z1_res[100], *z2_res[100], *z3_res[100];
double z1_el[100], z2_el[100], z3_el[100];
double z1_rv[100], z2_rv[100], z3_rv[100];
double z1_la[100], z2_la[100], z3_la[100];


double z1_bit1[100], z2_bit1[100], z3_bit1[100];
...
/* time difference calcultion fuction*/
...
void downloadZ2(void *ptr)
{
    ...
    // Add some requests
    // int NUM_HANDLES = aux->upper_limit;
    ...
    for (jTile = aux->lower_limit; jTile <= aux->upper_limit; jTile++)
    {
        ...
        z2_ss[cnt2] = (8.0 * volume);
        z2_st[cnt2] = ttime;
        z2_bit[cnt2] = bitrate;
        z2_bit1[cnt2] = bitrate1;
        z2_res[cnt2] = resolutionAdj;
        z2_el[cnt2] = elapsed;
        z2_rv[cnt2] = refAdj;
        z2_la[cnt2] = local_avg;
...
void downloadZ3(void *ptr)
{
    ...
    for (jTile = aux->lower_limit; jTile <= aux->upper_limit; jTile++)
    {
        ...
        z3_ss[cnt3] = (8.0 * volume);
        z3_st[cnt3] = ttime;
        z3_bit[cnt3] = bitrate;
        z3_bit1[cnt3] = bitrate1;
        z3_res[cnt3] = resolutionOut;
        z3_el[cnt3] = elapsed;
        z3_rv[cnt3] = refOut;
        z3_la[cnt3] = local_avg;
        ...
        for (j = 0; j < mlenj; j++)
        {
            ...
            if (output[i][j] == 1)
            {
                ...
                for (k = 1; k <= adjlen; k++)
                {
                    ...
                    // right
                    if (output[i][(j + k) % mlenj] == 3)
                        output[i][(j + k) % mlenj] = 2;
                    // up
                    if (output[(i + k) % mleni][j] == 3)
                        output[(i + k) % mleni][j] = 2;
                    ...
                    {
                        if (output[mleni - (-1 * (i - k))][j] == 3)
                            output[mleni - (-1 * (i - k))][j] = 2;
                    }
                    // left
                    if (j - k >= 0)
                    {
                    ...
                    {
                        if (output[i][mlenj - (-1 * (j - k))] == 3)
                            output[i][mlenj - (-1 * (j - k))] = 2;
                    }

                    // diagonal: (i+1)(j+1)
                    if (output[(i + k) % mleni][(j + k) % mlenj] == 3)
                        output[(i + k) % mleni][(j + k) % mlenj] = 2;

                    // diagonal:(i+1)(j-1)
                    if (j - k >= 0)
                    {
                        if (output[(i + k) % mleni][j - k] == 3)
                            output[(i + k) % mleni][j - k] = 2;
                    }
                    else
                    {
                        if (output[(i + k) % mleni][mlenj - (-1 * (j - k))] == 3)
                            output[(i + k) % mleni][mlenj - (-1 * (j - k))] = 2;
                    }

                    // diagonal:(i-1)(j+1)
                    if (i - k >= 0)
                    {
                        if (output[i - k][(j + k) % mlenj] == 3)
                            output[i - k][(j + k) % mlenj] = 2;
                    }
                    else
                    {
                        if (output[mleni - (-1 * (i - k))][(j + k) % mlenj] == 3)
                            output[mleni - (-1 * (i - k))][(j + k) % mlenj] = 2;
                    }
                    ...
                    {
                        if (output[i - k][mlenj - (-1 * (j - k))] == 3)
                            output[i - k][mlenj - (-1 * (j - k))] = 2;
                    }
                    else if (i - k < 0 && j - k >= 0)
                    {
                        if (output[mleni - (-1 * (i - k))][j - k] == 3)
                            output[mleni - (-1 * (i - k))][j - k] = 2;
                    }
                    else
                    {
                        if (output[mleni - (-1 * (i - k))][mlenj - (-1 * (j - k))] == 3)
                            output[mleni - (-1 * (i - k))][mlenj - (-1 * (j - k))] = 2;
                    }
                }
            }
        }
...
int main(int argc, char **argv)
{
    ...
    if (argc < 13)
    {
        printf("usage: cplayer <destination> <video_id> <segment_count> <uuid> <tracefile> <time_limit(sec)> <lines_i(4)> <column_j(8)> <error_rate> <num_threads> <heuristic> <buffer_size> <path>\n:");
        return (-1);
    }
   ...
   /* per tile/segment level metrics */
    char filename1[1000];
    filename1[0] = '\0';
    strcat(filename1, path);
    strcat(filename1, uuid);
    strcat(filename1, "-segment");
    ...
    ///////////////////////
     ...
     /* per-session level metrics */
    char filename2[1000];
    filename2[0] = '\0';
    strcat(filename2, path);
    strcat(filename2, uuid);
    strcat(filename2, "-session");
    ...
    if (curl)
    {
        ...
        while (i <= segment_count_i)
        {
            ...
            if (i > 1)
            {
                ...
                else if (refOut < 2120000) // 2.12 Mbps
                {
                ...
                else if (refOut < 2370000) // 2.37  mbps
                {
            ...
            for (jTile = 0; jTile < len_viewport; jTile++)
            {
                ...
                /* HTTP/2 please */
                curl_easy_setopt(curl1, CURLOPT_HTTP_VERSION, (long)CURL_HTTP_VERSION_1_1);
                ...
                curl_easy_getinfo(curl1, CURLINFO_SIZE_DOWNLOAD, &volume);
                curl_easy_getinfo(curl1, CURLINFO_TOTAL_TIME, &ttime);
                ...
                // fprintf(flog1, "%lf;%d;%s;%lf;%lf;%lf;%lf;%lf;vp\n", elapsed, i, resolutionVp, refVp, local_avg, bitrate, volume, ttime);
                // fflush(flog);

                z1_ss[cnt1] = (8.0 * volume);
                z1_st[cnt1] = ttime;
                z1_bit[cnt1] = bitrate;
                z1_bit1[cnt1] = bitrate1;
                z1_res[cnt1] = resolutionVp;
                z1_el[cnt1] = elapsed;
                z1_rv[cnt1] = refVp;
                z1_la[cnt1] = local_avg;
            ...
            memset(z1_ss, 0, sizeof(z1_ss));
            memset(z2_ss, 0, sizeof(z2_ss));
            memset(z3_ss, 0, sizeof(z3_ss));

            memset(z1_st, 0, sizeof(z1_st));
            memset(z2_st, 0, sizeof(z2_st));
            memset(z3_st, 0, sizeof(z3_st));

            memset(z1_bit, 0, sizeof(z1_bit));
            memset(z2_bit, 0, sizeof(z2_bit));
            memset(z3_bit, 0, sizeof(z3_bit));

            memset(z1_bit, 0, sizeof(z1_bit1));
            memset(z2_bit, 0, sizeof(z2_bit1));
            memset(z3_bit, 0, sizeof(z3_bit1));

            memset(z1_el, 0, sizeof(z1_el));
            memset(z2_el, 0, sizeof(z2_el));
            memset(z3_el, 0, sizeof(z3_el));

            memset(z1_rv, 0, sizeof(z1_rv));
            memset(z2_rv, 0, sizeof(z2_rv));
            memset(z3_rv, 0, sizeof(z3_rv));

            memset(z1_la, 0, sizeof(z1_la));
            memset(z2_la, 0, sizeof(z2_la));
            memset(z3_la, 0, sizeof(z3_la));

            memset(z1_res, '\0', sizeof(z1_res));
            memset(z2_res, '\0', sizeof(z2_res));
            memset(z3_res, '\0', sizeof(z3_res));
